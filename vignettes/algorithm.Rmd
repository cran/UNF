...
This vignette describes the UNF algorithm (Altman, Gill, McDonald 2003; Altman and King 2007; Altman 2008) and the R implementation thereof, which includes some peculiarities. The official specifications for various versions of UNF can be found elsewhere online. The algorithm is described in general terms [here](http://guides.dataverse.org/en/latest/developers/unf/index.html) and one can also find more specific descriptions of the [Version 3/4](http://guides.dataverse.org/en/latest/developers/unf/unf-v3.html), [Version 5](http://guides.dataverse.org/en/latest/developers/unf/unf-v5.html) and [Version 6](http://guides.dataverse.org/en/latest/developers/unf/unf-v6.html) algorithms.
...
Round numerics to *k* digits, where the default value of *k* is 7. (Note: In UNF versions <= 5, *k* was labeled *n*.) Then, convert those numerics to a character-class string containing exponential notation in the following form:
  ...
  Note (c): The [Dataverse](http://dataverse.org/) implementation of UNFv5 represents zero values (and logical `FALSE`) values as "+0.e-6" rather than the implied "+0.e+" (like logical `TRUE` values: "+1.e+"). This can be replicated in `unf5()` by adding the argument `dvn_zero = TRUE`.
